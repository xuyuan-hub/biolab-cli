...
- [x] API 响应模块：新增 `src/api_response.rs`，从 `client.rs` 移出响应解析函数，修正数组解析静默吞错（已完成，commit 4d6f949 / 13afc8b）
- [x] HTTP transport 模块：新增 `src/http.rs`，从 `client.rs` 移出 HTTP 请求构造、token header、multipart 上传（已完成，commit 8c23f85）
- [x] 服务层模块：新增 `src/services/` 拆分 users、orders、templates、inventory、lab 服务（已完成，commit 8c23f85）
- [x] Agent Skill 文档模块：拆分为 biolab-shared、biolab-orders、biolab-inventory、biolab-templates、biolab-lab、biolab-users、biolab-project、biolab-tashan 共 8 个技能（已完成，commits 81b1c0d / 167c76b / 02c1f0f）
- [ ] 输出契约模块：统一 text/json 输出 envelope，下载类/创建类/列表类命令输出策略一致 —— **未开始。原因：** 当前 `print_result` 已对 JSON/text 做基本区分，但 `orders download` 等命令直接 `println!` 未走统一输出契约。暂因优先级低于业务命令覆盖而搁置。
- [ ] 业务工作流命令：新增 `orders validate`、`orders preview`、`create-primer --from-template` 等命令 —— **未开始。原因：** 已有 `biolab orders create-primer <file>` 可通过模板 JSON 下单，validate/preview 属额外体验优化，暂无实际业务需求推动。
- [ ] 拆分认证流程：增加 Agent 友好的 `auth status --json`、`login --no-wait`、`callback/poll` —— **部分完成。** 已实现 `biolab login --background` 和 `biolab status`，满足基本 Agent split-flow 需求（commit 0b75178 / 79bd223），但 `login --no-wait` 和 callback 独立接口未实现。
- [ ] 拆分 `types.rs` 为领域类型模块，把状态、角色、订单类型收敛为 enum —— **未开始。原因：** 当前 `string_or_f64` 自定义反序列化已解决后端类型不稳定的问题，拆分 `types.rs` 属纯重构，暂不影响功能，优先级较低。
- [ ] 完善发布链路：补 checksums、安装脚本和更明确的二进制产物策略 —— **未开始。原因：** GitHub Actions 已有 4 平台构建和 auto-release，checksums 和安装脚本属额外工程化，暂无用户诉求推动。
