...
全部 18 个 endpoint 已实现（commits bbc0ca9 / 8d1662c）：

### Phase 1 — 高频操作（全部完成）
- [x] `biolab orders stats` — `src/services/orders.rs:get_order_stats` + `src/commands/orders.rs:Stats`
- [x] `biolab orders pending-approvals` — `list_pending_approvals` + `PendingApprovals`
- [x] `biolab orders approve <ID>` — `approve_order` + `Approve`
- [x] `biolab orders reject <ID>` — `reject_order` + `Reject`
- [x] `biolab inventory transactions <ID>` — `list_stock_transactions` + `Transactions`

### Phase 2 — 课题组视图（全部完成）
- [x] `biolab lab orders` — `list_lab_orders` + `Orders`
- [x] `biolab lab orders-stats` — `get_lab_order_stats` + `OrdersStats`
- [x] `biolab lab inventory` — `list_lab_inventory` + `Inventory`

### Phase 3 — 库存设置（全部完成）
- [x] `biolab inventory preferences` — `get_inventory_preferences` + `Preferences`
- [x] `biolab inventory set-preferences` — `set_inventory_preferences` + `SetPreferences`

### Phase 4 — 项目管理（全部完成）
- [x] `biolab projects list` — `list_projects` + `List`
- [x] `biolab projects get <ID>` — `get_project` + `Get`
- [x] `biolab projects create` — `create_project` + `Create`
- [x] `biolab projects update <ID>` — `update_project` + `Update`
- [x] `biolab projects members <ID>` — `list_project_members` + `Members`
- [x] `biolab projects add-member` — `add_project_member` + `AddMember`
- [x] `biolab projects remove-member` — `remove_project_member` + `RemoveMember`

### 未实现项（有意排除）
- [ ] Users admin APIs / signup / password recovery —— **有意不实现。** 方案明确规定 "must not be opened in the CLI yet"，等待显式需求。
- [ ] Audit / audit-log endpoints —— **有意不实现。** 方案明确排除，等待显式需求。
